...
Typical session: **3-4 people total**. System audio carries **multiple remote voices on one wire**, and that is why Listnr needs **speaker diarization**, not only “You vs Others.”
...
`SpeakerKit` (`PyannoteDiarizationOptions`) supports `numberOfSpeakers`, so for these meetings default the hint to **`numberOfSpeakers: 2-3` on Lane B** (remotes only), or auto-detect with a max of 4.
...
**Done when:** remote spans labeled separately and exported. (**DONE**, still to validate on a real 3-4 person call)
...
2. **Slightly gentler thresholds.** Speech start and hangover tuned for natural pauses.
3. **Diarization span padding.** ~350 ms before and ~200 ms after each SpeakerKit cut before STT.
...
Sibling folders `../parrot` and `../meetily` were local reference checkouts while developing. They are not part of this repository and nothing from them ships inside the Listnr binary.
