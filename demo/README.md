...
|---|---|---|---|
| `listnr-session.gif` | A real call transcribed live, `You` vs `Speaker 1...N` | `demo/record-session.sh` | Only a maintainer with a live call |
...
The session GIF is the one that matters and the only one not yet recorded. The
whole pitch is that the transcript is correct about who said what, and neither an
install log nor `--help` output demonstrates that.
...
## Why the session GIF cannot be automated
...
talk into the microphone, so it cannot run unattended.
